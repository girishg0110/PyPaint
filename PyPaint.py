...
def color_from_int(integ, color = (255,255,255)):
    if (integ == 1): #RED
        return (255,0,0)
    if (integ == 2): #ORANGE
        return (255,127,0)
...
def draw_menu(canvas, menu_color, menu_width, menu_font, dims):
    fill_background(canvas, invert_color(menu_color), (0, 0, menu_width, dims[1]))
    pygame.draw.line(canvas, menu_color, (0, 0), (0, dims[1]), 5)
    pygame.draw.line(canvas, menu_color, (menu_width, 0), (menu_width, dims[1]), 5)
    for i in range(11):
        pygame.draw.line(canvas, menu_color, \
            (0, menu_width*i), (menu_width, menu_width*i), 5)
    for i in range(10):
        menu_square = (menu_width//10, menu_width * i + menu_width//10, menu_width*8//10, menu_width*8//10)
...
def main():
    ...
    #COLOR MENU
    menu_color = (0,0,0) if background_color != (0,0,0) else (255,255,255)
    ...
    root = os.getcwd()
    instructions = str(background_color) #each line is (COLOR)(POINT1)(POINT2)(THICKNESS)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if (pygame.mouse.get_pressed()[0]):
                if (pygame.mouse.get_pos()[0] > dims[0]//10):
                    if (len(first_pair) == 0):
                        first_pair = pygame.mouse.get_pos()
                    else:
                        second_pair = pygame.mouse.get_pos()
                        pygame.draw.line(canvas, color, first_pair, second_pair, thickness)
                        instructions = instructions + "\n" + str(color) + str(first_pair) + str(second_pair) + str(thickness)
                        first_pair = ()
                else:
                    if (filling_mode):
                        background_color = color_from_int(pygame.mouse.get_pos()[1]//menu_width)
                        pygame.draw.rect(canvas, background_color, canvas_rect)
                        filling_mode = False
                        instructions = str(background_color)

                    color = color_from_int(pygame.mouse.get_pos()[1]//menu_width)

        #Hotkeys
        keys = pygame.key.get_pressed()
        if (keys[pygame.K_UP]):
            thickness = thickness + 1 if thickness < (min(dims)-50)//2 else thickness
        ...
        if (keys[pygame.K_s]):
            ...
            if (instruct_choice != 'n'):
                ...
                full_path = root + r"\Instructions" + r"\Instructions - " + instruct_name + ".txt"
                f = open(full_path, "w")
        ...
        if (keys[pygame.K_d]): #decode a text file
            ...
            if (os.path.exists(full_path)):
                ...
                background_color = (int(background_color_arr[0][1:]), int(background_color_arr[1]), int(background_color_arr[2][:-2]))
                pygame.draw.rect(canvas, background_color, canvas_rect)

                line_info_arr = f.readlines()[1:]
                for line in line_info_arr:
                    parsed_line = line_data_extract(line)
                    color = (parsed_line[0][0], parsed_line[0][1], parsed_line[0][2])
        ...
        pygame.draw.rect(canvas, background_color, (dims[0] - 10, thickness, 10, dims[1] - thickness))
